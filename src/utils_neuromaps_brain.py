...
#     cmap=my_cmap
    ...
    if parcellation_name=='glasser':
        conversion = "glasser_360_conte69"
    if parcellation_name=='schaefer':
        conversion = "schaefer_"+str(parcellation)+"_conte69"

    parc = parcel_to_surface(current_nodestrength, conversion)
    # Modify the value of the parcellation
    lh_parc_mod = parc[:len(parc)//2]
    rh_parc_mod = parc[len(parc)//2:]
...
def compose_full_view(remove_files=True, output_name='fig_final', save_svg=False, save_png=False):
    ...
    # im = plt.imread('{0}.svg'.format(output_name))
